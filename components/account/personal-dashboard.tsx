...
export function PersonalDashboard({
  ...
  if (!state.active && !auth.user) {
    return (
      <section className="pb-10 pt-5">
        <div className="rounded-[24px] border border-[#dfe6f4] bg-white p-5 shadow-[0_18px_46px_rgba(35,49,86,0.05)] dark:border-[#26324f] dark:bg-[#111a31]">
          <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-primary dark:text-[#ff9b4f]">
            Espacio personal gratis
          </p>
          <h2 className="mt-2 text-[26px] font-extrabold tracking-[-0.05em] text-[#081642] dark:text-white">
            Activa guardados, checklist e historial
          </h2>
          <p className="mt-3 max-w-[680px] text-[13px] font-semibold leading-6 text-[#66718f] dark:text-[#aeb9d4]">
            {auth.googleEnabled
          ...
          {!auth.googleEnabled ? (
            <button
              ...
              className="ml-0 mt-3 rounded-full border border-[#e3e9f4] bg-white px-5 py-3 text-[13px] font-bold text-[#52607f] dark:border-[#2a3654] dark:bg-[#121b32] dark:text-[#e8eeff] sm:ml-2"
            >
              Usar espacio local
            </button>
          ) : null}
          {!auth.configured ? (
            <p className="mt-3 rounded-[14px] bg-[#fff7ed] px-3 py-2 text-[11px] font-bold leading-5 text-[#8a4b12] dark:bg-[#3b2814] dark:text-[#ffb46f]">
              Falta configurar NEXT_PUBLIC_SUPABASE_URL,
              NEXT_PUBLIC_SUPABASE_ANON_KEY y Google en Supabase Auth.
            </p>
          ) : !auth.googleEnabled ? (
            <p className="mt-3 rounded-[14px] bg-[#eef2ff] px-3 py-2 text-[11px] font-bold leading-5 text-primary dark:bg-[#243461] dark:text-[#9fb1ff]">
  ...
  return (
    <section className="pb-10 pt-5">
        ...
        >
          {documentQueue.slice(0, 8).map(({ procedure, document }) => (
            <Link
              ...
              className="block rounded-[14px] bg-[#f7f9ff] p-3 dark:bg-[#121b32]"
            >
              <p className="text-[12px] font-extrabold text-[#081642] dark:text-white">
                {document.title}
              </p>
              <p className="mt-1 text-[11px] font-semibold text-[#66718f] dark:text-[#aeb9d4]">
                {procedure.title}
              </p>
            </Link>
          ))}
          ...
          {state.reminders.slice(0, 8).map((reminder) => {
            ...
            return (
              <button
                ...
                className="flex w-full items-start gap-3 rounded-[14px] bg-[#f7f9ff] p-3 text-left dark:bg-[#121b32]"
              >
                <span
                  className={
                    reminder.done
                ...
                <span>
                      <span className="block text-[12px] font-extrabold text-[#081642] dark:text-white">
                    {reminder.note}
                  </span>
                  <span className="mt-1 block text-[11px] font-semibold text-[#66718f] dark:text-[#aeb9d4]">
        ...
        >
          {monthlyAlerts.slice(0, 8).map((procedure) => (
            <Link
              key={procedure.slug}
              href={`/tramites/${procedure.slug}`}
              className="block rounded-[14px] bg-[#fff7ed] p-3 dark:bg-[#3b2814]"
            >
              <p className="text-[12px] font-extrabold text-[#081642] dark:text-white">
                {procedure.title}
              </p>
              <p className="mt-1 text-[11px] font-semibold text-[#8a4b12] dark:text-[#ffb46f]">
      ...
      <button
        onClick={clear}
        className="mt-5 inline-flex items-center gap-2 rounded-full border border-[#e3e9f4] bg-white px-4 py-2.5 text-[12px] font-bold text-[#52607f] dark:border-[#2a3654] dark:bg-[#121b32] dark:text-[#e8eeff]"
      >
...
function Metric({
  ...
  return (
    <div className="rounded-[20px] border border-[#e3e9f4] bg-white p-4 shadow-[0_12px_28px_rgba(35,49,86,0.035)] dark:border-[#26324f] dark:bg-[#111a31]">
      <Icon className="h-4 w-4 text-primary dark:text-[#ff9b4f]" />
      <p className="mt-3 text-[26px] font-extrabold tracking-[-0.05em] text-[#081642] dark:text-white">
        {value}
      </p>
      <p className="text-[12px] font-bold text-[#66718f] dark:text-[#aeb9d4]">{label}</p>
    </div>
  );
}
...
function Panel({
  ...
  return (
    <div className="rounded-[22px] border border-[#e3e9f4] bg-white p-4 shadow-[0_12px_28px_rgba(35,49,86,0.035)] dark:border-[#26324f] dark:bg-[#111a31]">
      ...
      <div className="grid gap-2">
        {hasChildren ? (
          ...
          <div className="rounded-[16px] bg-[#f7f9ff] p-4 dark:bg-[#121b32]">
            <p className="text-[13px] font-bold text-[#66718f] dark:text-[#aeb9d4]">{emptyTitle}</p>
...
function ProcedureRow({
  ...
  return (
    <div className="flex items-start justify-between gap-3 rounded-[14px] bg-[#f7f9ff] p-3 dark:bg-[#121b32]">
      <Link href={`/tramites/${procedure.slug}`} className="min-w-0">
        <p className="text-[12px] font-extrabold text-[#081642] dark:text-white">
          {procedure.title}
        </p>
        <p className="mt-1 line-clamp-2 text-[11px] font-semibold leading-5 text-[#66718f] dark:text-[#aeb9d4]">
          {procedure.summary}
        </p>
      </Link>
      <button
